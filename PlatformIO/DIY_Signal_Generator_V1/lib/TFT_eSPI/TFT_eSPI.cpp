...
void TFT_eSPI::writecommand(uint8_t c)
{
  ...
  tft_Write_8(c);
  //REG_WRITE(GPIO_OUT_W1TC_REG, clr_mask);
...
void TFT_eSPI::writedata(uint8_t d)
{
  ...
  //GPIO.out_w1tc = clr_mask;
  //ets_delay_us(1);
  //GPIO.out_w1ts = set_mask((uint8_t)(d)); 
  //NOP();NOP();NOP();NOP();
  //ets_delay_us(1);
  //WR_H;
  //ets_delay_us(1);
...
void TFT_eSPI::setWindow(int32_t x0, int32_t y0, int32_t x1, int32_t y1)
{
  ...
  // Column addr set
  DC_C; tft_Write_8(TFT_CASET); //ORIGINAL
  //writecommand(TFT_CASET); //Added for timings
  //DC_C; tft_Write_8(TFT_PASET);
  DC_D; tft_Write_32C(x0, x1); //ORIGINAL

  // Row addr set
  DC_C; tft_Write_8(TFT_PASET);//ORIGINAL
  //writecommand(TFT_PASET); //Added for timings
  //DC_C; tft_Write_8(TFT_CASET);
  DC_D; tft_Write_32C(y0, y1); //ORIGINAL

  DC_C; tft_Write_8(TFT_RAMWR);//ORIGINAL
...
void TFT_eSPI::fillRect(int32_t x, int32_t y, int32_t w, int32_t h, uint32_t color)
{
  ...
  setWindow(x, y, x + w - 1, y + h - 1);//ORIGINAL
  DC_D;
  pushBlock(color, w * h);
  CS_L;
  end_tft_write();
